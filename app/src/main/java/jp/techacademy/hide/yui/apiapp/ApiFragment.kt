...
// このApiFragmentクラスで実際のAPI通信や取得したJsonデータをApiResponseへの変換を行う
class ApiFragment: Fragment() {

    // RecyclerViewの表示のために必要となるAdapterクラス
    private val apiAdapter by lazy { ApiAdapter(requireContext()) }

    // handler プロパティは、別スレッドで動作させるために作成したもの
    // 描画はMainスレッドで行い、API通信の処理は別スレッドで行う必要があるため
    // 今回はAPI通信をした後に、そのデータをViewに反映させて描画させる必要がある
    // 通信してデータを取得しても、そこは別スレッドなので、Viewにデータを渡して描画するのにMainスレッドに処理を跨がせる必要がある
    ...
    // このメソッド内でAPI通信を行い、データを取得して、データを新しい状態に更新する
    private fun updateData(isAdd: Boolean = false) {
        ...
        // はじめにurlを作成してベースとなるURLにGETパラメータを追加して、取得するデータの条件を決める
        ...
        // clientは、Http通信を行う本体
        // HttpLoggingInterceptorは、ログに通信の詳細を出す準備
        ...
        // 実際にHttp通信を行います。enqueueの引数にCallbackを設定します。
        // Callback内で通信成功時と失敗時の処理を書きます。
        client.newCall(request).enqueue(object: Callback {
            // 失敗した時（onFailure（）メソッド）は、RecyclerViewを空のリストで更新しています
            ...
            // 成功した時（onResponse（）メソッド）は、取得したデータ（Json）をApiResponseというクラスに変換
            override fun onResponse(call: Call, response: Response) { // 成功時の処理
                // ApiFragmentでJsonからApiResponseに変換している
                ...
                // API通信して取得したデータを、Viewに反映させて描画する必要があるが
                // ここは別スレッドなので、Viewにデータを渡すためにMainスレッドに処理を跨がせる
                handler.post {
